                ...
                {datasNotExpend?.map((visualizacoesNotExpend) => {
                  return (
                    <TableRow
                      ...
                      key={visualizacoesNotExpend.created_at}
                    >
                      <TableCell className="max-w-[340px]">
                        {visualizacoesNotExpend.description}
                      </TableCell>
                      <TableCell>{format(parseISO(visualizacoesNotExpend.date), "dd/MM/yyyy")}</TableCell>
                      <TableCell className="text-green-600">
                        {priceFormatter.format(visualizacoesNotExpend.value)}
                      </TableCell>
                      <TableCell className="flex gap-2 font-bold ml-1">
                        <EditButton budgetName="Receita" id={visualizacoesNotExpend.id} />
              ...
              <TableBody>
                {datasExpend?.map((visualizacoesExpend) => {
                  return (
                    <TableRow
                      className="font-semibold"
                      key={visualizacoesExpend.created_at}
                    >
                      <TableCell>{visualizacoesExpend.description}</TableCell>
                      <TableCell>{format(parseISO(visualizacoesExpend.date), "dd/MM/yyyy")}</TableCell>
                      <TableCell className="text-red-600">
                        -{priceFormatter.format(visualizacoesExpend.value)}
                      </TableCell>
                      <TableCell className="flex gap-2 font-bold ml-1">
                        <EditButton budgetName="Despesa" id={visualizacoesExpend.id} />
